...
export default function Dashboard() {
  ...
  async function handleAdd(opp: Parameters<typeof fetch>[1] extends undefined ? never : object) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Freelance, pitches & job opportunities</p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="bg-white rounded-xl border shadow-sm p-4 mb-6 space-y-3">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            {TYPES.map(t => (
              <button key={t.value} onClick={() => setActiveType(t.value)}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${
                  activeType === t.value ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}>
                {t.label}
              </button>
            ))}
            ...
            {STATUSES.map(s => (
              <button key={s.value} onClick={() => setActiveStatus(s.value)}
                className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${
                  activeStatus === s.value ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                }`}>
                {s.label}
              </button>
            ))}
          ...
          <input
            ...
            placeholder="Search by title, pitch, skills, or author..."
            value={search}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 text-gray-400 gap-3">
            ...
            <p className="text-sm">No opportunities yet. Add one to get started!</p>
          </div>
        ) : (
