...
import { Target as TargetIcon, Wrench, FolderOpen, Edit2, Save, X, Plus, Trash2, ChevronDown, ChevronUp, CheckCircle, Clock, PauseCircle } from "lucide-react";
...
function TargetCard({ target, onUpdate, onDelete }: {
  ...
  const [form, setForm] = useState({ title: target.title, description: target.description ?? "", duration: target.duration ?? "", status: target.status });

  useEffect(() => {
    if (expanded && !loaded) {
      fetch(`/api/targets/${target.id}/comments`).then(r => r.json()).then(d => { setComments(d); setLoaded(true); });
    }
        ...
        {editing ? (
          <div className="mt-3 space-y-2">
            ...
            <input className="w-full border rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="Duration (e.g. Q3 2025, 3 months)" value={form.duration} onChange={e => setForm(f => ({ ...f, duration: e.target.value }))} />
          </div>
        ) : (
          target.description && <p className="mt-2 text-sm text-gray-500 leading-relaxed">{target.description}</p>
        )}
...
export default function HomePage() {
  ...
  async function addTarget() {
    ...
    const res = await fetch("/api/targets", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: newTarget.title, description: newTarget.description || null, duration: newTarget.duration || null }),
    });
    if (res.ok) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-10">
        ...
        <div className="bg-white rounded-xl border shadow-sm p-5">
          {editingMission ? (
            <div className="space-y-3">
              ...
              />
              <div className="flex gap-2">
                <button onClick={saveMission} className="flex items-center gap-1 px-4 py-1.5 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 transition-colors">
                  <Save size={13} /> Save
                </button>
      ...
      <section>
        ...
        {showAddTarget && (
          <div className="bg-white rounded-xl border shadow-sm p-4 mb-4 space-y-3">
            <input className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="Target title *" value={newTarget.title} onChange={e => setNewTarget(f => ({ ...f, title: e.target.value }))} autoFocus />
            <textarea className="w-full border rounded-lg px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-indigo-500"
              rows={2} placeholder="Description (optional)" value={newTarget.description} onChange={e => setNewTarget(f => ({ ...f, description: e.target.value }))} />
            <input className="w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="Duration (e.g. Q3 2025, 6 weeks)" value={newTarget.duration} onChange={e => setNewTarget(f => ({ ...f, duration: e.target.value }))} />
            <div className="flex gap-2">
        ...
        {targets.length === 0 ? (
          <p className="text-gray-400 text-sm italic">No targets yet. Add one above.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {targets.map(t => (
              <TargetCard key={t.id} target={t} onUpdate={updateTarget} onDelete={deleteTarget} />
            ))}
          </div>
        )}
      </section>
